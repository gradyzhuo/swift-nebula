...
///
/// Call this to forward the matter to the next middleware (or the core
/// dispatch handler). You can inspect or replace the matter before
...
/// Each middleware receives the incoming ``Matter`` and a `next`
/// closure that forwards to the next middleware in the chain.
///
/// ```swift
/// // Pre + post processing
/// public struct LoggingMiddleware: NMTMiddleware {
///     public func handle(_ matter: Matter, next: NMTMiddlewareNext) async throws -> Matter? {
///         print("→ \(matter.type)")
///         let reply = try await next(matter)
